...
| `.opencode/` | yes | OpenCode agent definitions for the `coder`, `reviewer` and `rules-checker` roles under `agents/`. The reviewer is read-only, as in `.claude/agents/`; Skills load from `.claude/skills/`. |
...
| `public/items/` | Card artwork, generated by `tools/item-assets.ps1` and `tools/tech-assets.ps1`. |
...
| `tech-assets.ps1` | Copies tech card art into the same `packages/web/public/items/` folder, including Space Flight. |
