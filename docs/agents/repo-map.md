...
Fastify over the engine. Holds no game rules: each route loads state, calls one
pure function, saves the result, and answers with a projection.
...
| `test/` | Vitest through `fastify.inject`, no network. |
