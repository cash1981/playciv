...
- **"Only log the last change" is a single-entry lookback, not a log rewrite (issue #71).** `appendRollingArenaLog` (in `arena.ts`) drops the immediately preceding log entry before appending a new one, but only when that entry was the same kind of adjustment (matched by a tag prefix) on the same card (matched by carrying the card on the log entry's existing `item` field — arena units are already fully public, so this discloses nothing new). It never looks further back than one entry. This covers the actual clutter source (a player nudging one unit's position, or toggling a kill, several times in a row) without the complexity of a general "replace any prior entry for this subject" mechanism, which nothing else in the codebase does either.
