...
Read the public board pieces, resolve the owner from the shared Wonders area's
piece `playerId`, and render a compact panel alongside the existing game
panels. Derive the coin cap from whether the player's pieces in the Wonders
area include The Internet, and pass that effective limit through the existing
coin counter UI.
...
- [ ] Wonders in play are listed with the owning player's username.
...
- [ ] Browser verified on the game page.
- [ ] No private information is added to the projection.
