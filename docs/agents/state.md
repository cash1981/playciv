...
- **Issue #43 Governments.** Player status now stores a public government per player, defaults and migrates to Despotism, applies the Rome/Russia/Japan starting exceptions, and lets any member update the shared value with a public log entry. The panel includes the supplied artwork and accessible text for all eight *Wisdom and Warfare* cards while deliberately leaving their effects unenforced. Spectator and replay controls are read-only; browser-verified at desktop and mobile widths and review-approved.
