<#
Converts gamedata-faf-waw.xlsx into a raw cell dump in JSON.

The aim is not to interpret game data here, but to reproduce EXACTLY what
Apache POI handed ItemReader.java, so the TypeScript port can reuse the same
filters.

So Apache POI's Cell.toString() is reproduced:
  BLANK    -> ""
  FORMULA  -> the formula text (hence "RAND()" in notRandomPredicate)
  NUMERIC  -> Double.toString(value), so 12 becomes "12.0" and 1.3 stays "1.3"
  STRING   -> the string value (rich text runs are concatenated)
  BOOLEAN  -> "TRUE"/"FALSE"
  ERROR    -> the error text

Output: a dense 2D grid per sheet. Row 0 is included (ItemReader filters it
itself through rowNotZeroPredicate), and columns are NOT compacted here — Java
filters each column independently, and the port has to reproduce that.
#>
param(
...
function Read-ZipEntry([System.IO.Compression.ZipArchive] $zip, [string] $name) {
    ...
    if ($null -eq $entry) { throw "Could not find '$name' in the archive" }
    $reader = New-Object System.IO.StreamReader($entry.Open())
    try { $reader.ReadToEnd() } finally { $reader.Dispose() }
}

# Java Double.toString: always at least one decimal, shortest round-tripping form.
...
try {
    # --- sharedStrings: concatenate every <t> in each <si> (rich text runs) ---
    $sharedStrings = @()
    ...
    foreach ($sheet in (Select-Xml -Xml $workbookXml -XPath '/m:workbook/m:sheets/m:sheet' -Namespace $ns)) {
        ...
        Write-Host ("  {0,-16} {1,3} rows" -f $sheetName, $rows.Count)
    }
} finally {
    $zip.Dispose()
}
...
$payload = [ordered] @{
    ...
    note     = 'Generated by tools/xlsx-to-json.ps1. Do not edit by hand.'
    sheets   = $sheets
}
...
Write-Host "`nWrote $Out ($([math]::Round((Get-Item $Out).Length / 1024, 1)) KB)"
